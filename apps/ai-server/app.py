...
# Báo lỗi ngay lập tức nếu quên tạo file .env hoặc quên cấu hình key
if not GENAI_API_KEY:
    raise ValueError("LỖI: Chưa tìm thấy GENAI_API_KEY trong file .env. Vui lòng kiểm tra lại!")
...
# Sửa lại: Bắt đầu bằng 3 dấu nháy kép """
DOCUMENT_CONTEXT = """
...
# --- 4. KHAI BÁO LUẬT THÉP CHO AI (SYSTEM PROMPT) ---
SYSTEM_PROMPT = f"""
Bạn là trợ lý ảo thông minh của hệ thống Quản lý Nghiên cứu Khoa học (NCKH) - Đại học Thủy Lợi (TLU).
Nhiệm vụ của bạn là giải đáp thắc mắc của sinh viên về NCKH một cách thân thiện, chuyên nghiệp và ngắn gọn.

NGUYÊN TẮC TỐI THƯỢNG (BẮT BUỘC PHẢI TUÂN THỦ):
1. BẠN CHỈ ĐƯỢC PHÉP sử dụng thông tin trong phần <TÀI LIỆU> dưới đây để trả lời câu hỏi.
2. TUYỆT ĐỐI KHÔNG tự sáng tạo, không bịa đặt, và không sử dụng kiến thức bên ngoài mạng internet.
3. Nếu sinh viên hỏi một vấn đề KHÔNG CÓ thông tin trong phần <TÀI LIỆU>, bạn BẮT BUỘC phải trả lời: "Xin lỗi, thông tin này hiện chưa có trong cơ sở dữ liệu của mình. Bạn vui lòng liên hệ Giảng viên hướng dẫn hoặc phòng Khoa học Công nghệ để được giải đáp chính xác nhất nhé!"
...
def chat():
    ...
    try:
        # Ghép System Prompt với câu hỏi của người dùng
        ...
        # In log ra terminal để bạn dễ debug
        print(f"Người dùng hỏi: {user_message}")
        print(f"Bot trả lời: {reply[:100]}...") 
