...
import { BellOutlined, FireOutlined, MessageOutlined } from "@ant-design/icons";
...
const StudentDashboard: React.FC = () => {
  ...
  const fetchDashboardData = useCallback(async () => {
    ...
    try {
      ...
      const projectsArray = Array.isArray(projectRes.data)
        ? projectRes.data
    ...
    try {
      ...
      const recruitList = Array.isArray(recruitRes.data)
        ? recruitRes.data
        : recruitRes.data.data || [];
  ...
  useEffect(() => {
    ...
    socket.on("receiveMessage", (newMessage: any) => {
      ...
      if (
        ...
        activeChat?.roomId !== newMessage.roomId
      ) {
      ...
      <Row gutter={[24, 24]}>
        <Col xs={24} lg={16}>
          ...
          <Card
            title={
              <span>
                <MessageOutlined /> Tin nhắn chờ phản hồi
              </span>
            }
            style={{
              borderRadius: "12px",
          ...
          >
            <List
              ...
              renderItem={(room: any) => {
                const partner =
                  room.user1Id === currentUser.id ? room.user2 : room.user1;
                const lastMsg = room.messages && room.messages[0];
                return (
                  <List.Item
                    style={{ cursor: "pointer" }}
                    onClick={() =>
                      setActiveChat({ roomId: room.id, receiver: partner })
                    }
                  >
                    <List.Item.Meta
                      avatar={
        ...
        <Col xs={24} lg={8}>
          <Card
            title={
              <span>
                <FireOutlined style={{ color: "#ff4d4f" }} /> Tuyển đồng đội
              </span>
            }
